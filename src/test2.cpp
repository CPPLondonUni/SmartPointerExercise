...
int main(int argc, char** argv)
{
    ...
    if(!provider.GetRandomData())
        ...
        std::cerr << "Test 2 passed\n";if(provider.GetRandomData()[randomDataBlockSize - 1] != provider.GetRandomDataAt(randomDataBlockSize - 1) ||
        provider.GetRandomData()[0] != provider.GetRandomDataAt(0))
        std::cerr << "GetRandomData and GetRandomDataAt doesnt return the same number\n";
    else{
